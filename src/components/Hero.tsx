    ...
    <section id="home" ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-50 to-white dark:from-[#0A0F1C] dark:to-[#1A1F2E] pt-24">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-16 py-20">
        ...
        <div className="text-center mb-16">
          ...
          {/* Subheadline */}
          <motion.p 
            className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-600 dark:text-gray-300 max-w-4xl mx-auto leading-relaxed mb-8 px-4 sm:px-6 md:px-0 text-center"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.4 }}
          >
            <span className="block sm:inline">Your website is your first impression.</span>
            <span className="block sm:inline"> We make sure it's unforgettable,</span>
            <span className="block sm:inline"> high-converting, and built to dominate.</span>
          </motion.p>
          ...
          >
            <a
              ...
              className="group inline-flex items-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl hover:shadow-lg hover:shadow-purple-500/25 transition-all duration-300 hover:scale-105 relative overflow-hidden text-sm sm:text-base"
            >
          ...
          <motion.div
            ...
            className="mb-16"
          >
            <p className="text-gray-500 dark:text-gray-400 mb-8 text-xs sm:text-sm px-4 sm:px-0 text-center">Trusted by industry leaders worldwide</p>
            ...
            <div className="relative overflow-hidden px-4 sm:px-0">
              <div className="flex animate-scroll space-x-4 sm:space-x-8">
                ...
                {clientLogos.map((client, index) => (
                  <motion.div 
                    key={`first-${index}`}
                    className="flex-shrink-0 w-16 h-10 sm:w-20 sm:h-12 md:w-24 md:h-16 bg-white/50 dark:bg-white/10 rounded-lg flex items-center justify-center backdrop-blur-sm border border-gray-200 dark:border-white/10 hover:bg-white/70 dark:hover:bg-white/20 transition-all duration-300"
                    whileHover={{ scale: 1.05 }}
                  >
                    <span className="text-gray-700 dark:text-white font-bold text-xs sm:text-sm md:text-lg">{client.logo}</span>
                  </motion.div>
                ))}
          ...
          >
            <div className="inline-flex items-center space-x-2 sm:space-x-3 px-3 sm:px-4 md:px-6 py-2 sm:py-3 bg-white/50 dark:bg-white/10 backdrop-blur-sm rounded-full border border-gray-200 dark:border-white/10">
              <span className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm">Specializing in:</span>
              <div className="text-xs sm:text-sm md:text-lg font-semibold text-orange-500 h-4 sm:h-5 md:h-6 overflow-hidden">
                <motion.div 
                  className="transition-transform duration-500 ease-in-out"
                  style={{ transform: `translateY(-${currentServiceIndex * (window.innerWidth < 640 ? 16 : window.innerWidth < 768 ? 20 : 24)}px)` }}
                >
                  {services.map((service, index) => (
                    <div key={index} className="h-4 sm:h-5 md:h-6 flex items-center text-center">
                      {service}
                    </div>
                  ))}
                </motion.div>
        ...
        >
          <div className="w-4 h-6 sm:w-5 sm:h-8 md:w-6 md:h-10 border-2 border-gray-400 dark:border-white/30 rounded-full flex justify-center relative animate-bounce">
            <div className="w-1 h-1 sm:h-2 md:h-3 bg-gradient-to-b from-orange-500 to-transparent rounded-full mt-1 sm:mt-2 animate-pulse"></div>
          </div>
